...
class Product
{
    ...
    public function productInsert($data, $file)
    {
        ...
        } elseif ($productName == "" || $categoryId == "" || $brandId == "" || $body == "" || $price == "" || $type == "") {
            $msg = "<span class='error'>Fields must not be empty!</span>";
            return $msg;
        } else {
            move_uploaded_file($file_temp, $uploaded_image);
            $query = "INSERT INTO product(productName, categoryId, brandId, body, price, image, type) VALUES('$productName', '$categoryId', '$brandId', '$body', '$price', '$uploaded_image', '$type')";
    ...
    public function getAllProduct()
    {
        $query = "SELECT p.*, c.categoryName, b.brandName
                    ...
                    WHERE p.categoryId = c.categoryId AND p.brandId = b.brandId
    ...
    public function productUpdate($data, $file, $proid)
    {
        ...
        if ($productName == "" || $categoryId == "" || $brandId == "" || $body == "" || $price == "" || $type == "") {
            $msg = "<span class='error'>Fields must not be empty!</span>";
            return $msg;
        } else {
            if (!empty($file_name)) {
    ...
    {
        $query = "SELECT * FROM product WHERE type = '1' ORDER BY productId DESC LIMIT 4";
        $result = $this->db->select($query);
        return $result;
    }
    ...
    public function getSingleProduct($proId)
    {
        $query = "SELECT p.*, c.categoryName, b.brandName
                    FROM product AS p, category AS c, brand AS b
                    WHERE p.categoryId = c.categoryId AND p.brandId = b.brandId AND p.productId = '$proId'";
    ...
    {
        $query = "SELECT * FROM product WHERE brandId = '5' ORDER BY productId DESC LIMIT 4";
        $result = $this->db->select($query);
        return $result;
    }
    public function getBrandProductSamsung()
    {
        $query = "SELECT * FROM product WHERE brandId = '1' ORDER BY productId DESC LIMIT 4";
        $result = $this->db->select($query);
        return $result;
    }
    public function getBrandProductCanon()
    {
        $query = "SELECT * FROM product WHERE brandId = '4' ORDER BY productId DESC LIMIT 4";
        $result = $this->db->select($query);
        return $result;
    }
    ...
    {
        $cquery = "SELECT * FROM wlist WHERE cmrId = '$cmrId' AND productId = '$proId'";
        $check = $this->db->select($cquery);
        if ($check) {
            $msg = "<span class='error'>Product Already Added to Compare.</span>";
            return $msg;
        ...
        $pquery = "SELECT * FROM product WHERE productId = '$proId'";
